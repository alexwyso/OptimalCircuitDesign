...
class OptimalCircuitDesignProblem():
    ...
    def create_variables(self):
        ...
        for i in range(maxLengthCircuit):
            solution[i] = model.NewIntVar(-1 * numResistors, maxResistor, ' ')
    ...
    def addResistorSetConstraint(self):
        ...
        for i in range(numResistors + 2):
            resistorsListOfList.append([-1 * i])
    ...
    def addSingleUseConstraint(self):
        ...
        for i in range(maxLengthCircuit):
            for j in range(maxLengthCircuit):
                if (i != j):
                    model.Add(self.solution[i] != self.solution[j]).OnlyEnforceIf(isOp[i].Not())           
    ...
    def addValidOperatorsConstraint(self):
        ...
        for i in range(1, maxLengthCircuit):
            ...
            # Subtract one for series operator values, as it takes two elements
            # off the stack and adds one back
            model.Add(counter[i] == counter[i - 1] - 1).OnlyEnforceIf(isSeries[i])

            # Add solutions[i] + 1 for parallel operator values, as it takes 
            # -1 * solutions[i] elements off the stack and adds one back
            model.Add(counter[i] == counter[i - 1] + solution[i] + 1).OnlyEnforceIf(isParallel[i])
    ...
    def minimizeError(self):
        ...
        model.Add(stack[0][0] == solution[0])
        ...
        for i in range(1, maxLengthCircuit):
            ...
            model.Add(stack[i] == stack[i - 1]).OnlyEnforceIf(isResistor[i])

            model.Add(stack[i][i] == solution[i]).OnlyEnforceIf(isResistor[i])
            ...
            #model.Add(equivRes[i] == equivRes[i - 1] - 1).OnlyEnforceIf(isSeries[i])
            #r1 = stack.pop()
            #r2 = stack.pop()
            #stack.append(r1+r2)
            ...
            #model.Add(equivRes[i] == equivRes[i - 1] + solution[i] + 1).OnlyEnforceIf(isParallel[i])
            #curr = []
            #for _ in range(abs(n)):
            #    curr.append(stack.pop())
            #inverses = [(1 / resistor) for resistor in curr]
            ...
            # Copy the stack over 
            model.Add(stack[i] == stack[i - 1]).OnlyEnforceIf(isEmpty[i])

        # edit everything to be times 1000 #
        #error = self.model.NewIntVar(0, maxResistor * numResistors, 'error')
        #error = abs((10000 * target) - (10000 * stack[maxLengthCircuit - 1][0]]))
